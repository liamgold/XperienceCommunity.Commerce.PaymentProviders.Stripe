...
{
    options.ApiKey = builder.Configuration["Stripe:ApiKey"] ?? Environment.GetEnvironmentVariable("STRIPE_API_KEY") ?? "";
    options.WebhookSecret = builder.Configuration["Stripe:WebhookSecret"] ?? Environment.GetEnvironmentVariable("STRIPE_WEBHOOK_SECRET");
});

// Register order payments service
builder.Services.AddScoped<XperienceCommunity.Commerce.PaymentProviders.Core.IOrderPayments, DancingGoat.Commerce.Payment.OrderPaymentsService>();
