...
public sealed class DancingGoatCheckoutController : Controller
{
    ...
    public async Task<IActionResult> ConfirmOrder(CustomerViewModel customer, CustomerAddressViewModel billingAddress, ShippingAddressViewModel shippingAddress, string languageName, PaymentShippingViewModel paymentShipping, CancellationToken cancellationToken)
    {
        ...
        var shippingPrice = shipping.FirstOrDefault(s => s.ShippingMethodID == shippingMethodId)?.ShippingMethodPrice ?? 0;
        var grandTotal = totalPrice + shippingPrice;

        // Convert to minor units (e.g., cents)
        var amountMinor = (long)(grandTotal * 100);
        ...
        var orderSnapshot = new OrderSnapshot(
            ...
            Currency: "GBP", // TODO: Make this configurable
            CustomerEmail: customer.Email,
            SuccessUrl: new Uri($"{baseUrl}/{languageName}/payment-success?orderNumber={orderNumber}"),
            CancelUrl: new Uri($"{baseUrl}/{languageName}/payment-cancelled?orderNumber={orderNumber}")
        );

        var result = await paymentGateway.CreateOrReuseSessionAsync(orderSnapshot, cancellationToken);

        // Discard the shopping cart after creating the Stripe session
